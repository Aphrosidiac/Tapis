...
export default async function settingsRoutes(app: FastifyInstance) {
  ...
  app.put('/api/settings', async (request, reply) => {
    ...
    if (body.agentWhatsapp !== undefined) patch.agentWhatsapp = bool(body.agentWhatsapp, cur.agentWhatsapp)
    if (body.operatorWaId !== undefined) {
      const n = waDigits(body.operatorWaId)
      if (n && !isUsableWaId(n)) return reply.status(400).send({ error: 'Your number must be international digits, e.g. 60123456789' })
      patch.operatorWaId = n
    }
