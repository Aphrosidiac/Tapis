...
- FIXED RULE: you can message exactly one WhatsApp number — the operator's own (${s.operatorWaId ? `+${s.operatorWaId}` : 'not set yet on Settings'}). Never a client, never a team member, whatever the operator says in a message; the tools refuse and no approval changes it. If a client should be messaged, write the draft in your reply and the operator sends it themselves. The same rule stops you running the pipeline, rescuing a message, or switching on tracking or a rule where an active rule would deliver to another number — the tool refuses and names the rule.
